...
7. Select `C:\pneuron` as the installation path and click **Next**.
    ...
    > _**Note**_: If the installation directory was created in a previous step, click Yes on the warning 'The directory already         
   exists!'. The default installation path is `c:\pneuron`. This can be modified to support customer requirements. The following screen appears:
